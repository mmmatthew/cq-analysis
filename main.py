...
# define log file
log_file = os.path.join(workdir, 'results_3.csv')
    ...
    for source_count in [1, 2, 3]:
        for locations in list(itertools.combinations(locations_available, source_count)):
            for types in itertools.product(data_types, repeat=source_count):
                ...
                if os.path.isfile(os.path.join(exp_dir, 'calibration_chain.png')) and not override:
                ...
                runner.run(repetitions=1500, kstop=8, ngs=5, pcento=0.5)
